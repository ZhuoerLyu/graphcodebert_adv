...
class Attacker():
    ...
    def greedy_attack(self, example, substitutes, code, sim_score_dict):
        '''
        return
            original program: code
            program length: prog_length
        ...
        for name in names_positions_dict.keys():
            ...
            total_score = total_score*(len(positions)/len(importance_score))
        ...
        all_potential_replaced_words = list(names_to_importance_score.keys())
        try:
            all_sorted_vuln_line = sim_score_dict[code[0]]["key"]
        except KeyError as keyerror:
            return code, prog_length, adv_code, true_label, orig_label, temp_label, -3, variable_names, None, None, None, None
        order_targeted_word = []
        for vuln_line in all_sorted_vuln_line:
            for target_word in all_potential_replaced_words:
                if (target_word in vuln_line) and (target_word not in order_targeted_word):
                    order_targeted_word.append(target_word)

            

        for name_and_score in order_targeted_word:
            tgt_word = name_and_score
