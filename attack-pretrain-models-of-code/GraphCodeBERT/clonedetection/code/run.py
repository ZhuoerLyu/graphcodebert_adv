...
import sys
sys.path.append("D:\\adv_ml\\attack-pretrain-models-of-code\GraphCodeBERT\clonedetection\code\parser_lang")
from parser_lang import DFG_python,DFG_java,DFG_ruby,DFG_go,DFG_php,DFG_javascript
from parser_lang import (remove_comments_and_docstrings,
                   tree_to_token_index,
                   index_to_code_token,
...
for lang in dfg_function:
    LANGUAGE = Language('D:\\adv_ml\\attack-pretrain-models-of-code\GraphCodeBERT\clonedetection\code\parser_lang\my-languages.so', lang)
    parser = Parser()
...
def extract_dataflow(code, parser,lang):
    ...
    for i in range(len(code.split("\n"))):
        try:
            ...
            deleted_line = splited_code[i]
            modefied_code = splited_code[:i] + splited_code[i+1:]
            modefied_code = "\n".join(modefied_code)

            tree = parser[0].parse(bytes(modefied_code,'utf8'))    
            root_node = tree.root_node  
            tokens_index=tree_to_token_index(root_node)     
            modefied_code=modefied_code.split('\n')
            code_tokens=[index_to_code_token(x,modefied_code) for x in tokens_index]  
            index_to_code={}
            for idx,(index,temp_code) in enumerate(zip(tokens_index,code_tokens)):
                index_to_code[index]=(idx,temp_code)  
            try:
            ...
            dfg=[]
        masked_line[str(i)+"*****"+deleted_line] = {"code_tokens":code_tokens, "dfg":dfg}
    ...
    for d in DFG:
        if d[1] in indexs:
            new_DFG.append(d)
    dfg=new_DFG
    complete_code = {"code_tokens":code_tokens, "dfg":dfg}
    cosine_sim = []
    key = []
    for deleted_line in  masked_line.keys():
        key.append(deleted_line)
        code_tokens = masked_line[deleted_line]["code_tokens"]
        dfg = masked_line[deleted_line]["dfg"]

        overall_sim = similarity_dfg(complete_code["dfg"], dfg)
        cosine_sim.append(overall_sim)

    cosine_sim, key = (list(t) for t in zip(*sorted(zip(cosine_sim, key))))
    return code_tokens, dfg, cosine_sim, key

def similarity_dfg(dfg_complete, dfg2_maskedline):
    token_dfg_complete = []
    token_dfg_maskedline = []

    for single_set in dfg_complete:
        for single_item in single_set:
            if single_item != "comesFrom":
                token_dfg_complete.append(str(single_item))
    for single_set in dfg2_maskedline:
        for single_item in single_set:
            if single_item != "comesFrom":
                token_dfg_maskedline.append(str(single_item))
    from collections import Counter
    token_dfg_complete_counter =Counter(token_dfg_complete) 
    token_dfg_maskedline_counter =Counter(token_dfg_maskedline) 
    ...
    cosine_sim = counter_cosine_similarity(token_dfg_complete_counter, token_dfg_maskedline_counter)
    legth_sim = length_similarity(token_dfg_complete_counter, token_dfg_maskedline_counter)
    overall_sim = cosine_sim*legth_sim
    return overall_sim
...
def convert_examples_to_features(item):
    ...
    for url in [url1,url2]:
        if url not in cache:
            ...
            #extract data flow
            code_tokens,dfg, cos_sim, key=extract_dataflow(func,parser,'java')
            sim_score_dict[url]["cos_sim"] = cos_sim
            sim_score_dict[url]["key"] = key
    ...
    return InputFeatures(source_tokens_1,source_ids_1,position_idx_1,dfg_to_code_1,dfg_to_dfg_1,
                   source_tokens_2,source_ids_2,position_idx_2,dfg_to_code_2,dfg_to_dfg_2,
                     label,url1,url2), sim_score_dict
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train'):
        ...
        cache_score_path = os.path.join(folder, 'score_cached_{}'.format(postfix))
        # 保存下对应的code1和code2
        code_pairs_file_path = os.path.join(folder, 'cached_{}.pkl'.format(postfix))
        code_pairs = []
        try:
            self.examples = torch.load(cache_file_path)
            self.sim_score_dict = torch.load(cache_score_path)
            ...
            for x in data:
                single_example, single_sim_score_dict= convert_examples_to_features(x)
                self.examples.append(single_example)
                self.sim_score_dict.append(single_sim_score_dict)
            torch.save(self.examples, cache_file_path)
            torch.save(self.sim_score_dict, cache_score_path)
    ...
    def __getitem__(self, item):
        ...
        return (torch.tensor(self.examples[item].input_ids_1),
                ...
                torch.tensor(self.examples[item].label)), self.sim_score_dict
