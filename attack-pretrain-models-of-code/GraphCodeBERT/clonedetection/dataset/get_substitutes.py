...
def main():
    ...
    eval_data = []
    args.store_path =  "./test_subs_0_500.jsonl" 
    args.base_model = "microsoft/graphcodebert-base"
    args.eval_data_file = "./test_sampled.txt"
    args.block_size =  512
    args.index = [0, 4000]
    ...
    sim_score_dict_path = "D:\\adv_ml\\attack-pretrain-models-of-code\GraphCodeBERT\clonedetection\dataset\score_cached_test_subs_0_500"
    sim_score_dict = torch.load(sim_score_dict_path)
    
    with open('D:\\adv_ml\\attack-pretrain-models-of-code\GraphCodeBERT\clonedetection\dataset\data.jsonl') as f:
        for line in f:
            line=line.strip()
            js=json.loads(line)
            url_to_code[js['idx']]=js['func']
    
    with open("D:\\adv_ml\\attack-pretrain-models-of-code\GraphCodeBERT\clonedetection\dataset\\test_sampled.txt") as f:
    ...
    with open(args.store_path, "a") as wf:
        for item in tqdm(eval_data):
            ...
            for i in range(len(sim_score_dict)):
                if item["id1"] in sim_score_dict[i].keys():
                    try:
                        cos_sim = sim_score_dict[i][item["id1"]]['cos_sim']
                        cos_sim = [1/item for item in cos_sim]
                        lines = item["code1"].split("\n")[:-1]
                        extended_cos_sim = []
                        for line_num in range(len(lines)):
                            line_code = lines[line_num]
                            num_sub_token = 0
                            extended_cos_sim += cos_sim[line_num:line_num+1]*len(line_code.split(" "))
                        
                        # Pus mask on
                        input_ids_cpu = input_ids_.cpu().detach().numpy()[0]
                        mask_pos = [1]*word_predictions.shape[1]
                        for j in range(len(input_ids_cpu)):
                            input_id = input_ids_cpu[j]
                            mask_pos[input_id] = extended_cos_sim[j]
                        import numpy as np
                        mask_pos = np.tile(mask_pos,(word_predictions.shape[0],1))
                        word_predictions = torch.mul(word_predictions,torch.tensor(mask_pos).to("cuda"))
                        break
